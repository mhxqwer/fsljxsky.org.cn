...
(function ($) {
    ...
    $.SortUrl = function (val) {
        var parem = "sort_order";
        var url = window.location.href.toLowerCase().replace("#message", "");
        if ($.GetUrl(parem) == "")
            url += (url.indexOf("?") == -1 ? "?" : "&") + parem + "=" + val;
        else
            url = $.ChangeUrlArg(window.location.href.toLowerCase(), parem, $.GetUrl(parem) == val ? (val - 1) : val);
        location.href = url;
    },
    $.SubUrl = function (val) {
        //得到参数
    ...
    $.SubString = function (str, start, end) {
        ...
        if (istart == -1) return "";
        var iend = str.indexOf(end, istart + start.length);
        str = str.substring(istart + start.length, iend);
        return str;
    },

    $.IsNullOrEmpty = function (txt) {
        if (txt == undefined || txt == null || txt.toString() == "" || txt.toString().indexOf("undefined")!=-1)
            return "";
        else
            return txt.toString();
    },
	//时间比较 date1 当前时间 date2比较时间
